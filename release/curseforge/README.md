...
这些文件用于准备 PartyTargetWatch 0.4.0 的 CurseForge 项目页与版本说明。
...
| `changelog.md` | 0.4.0 更新说明；英文在前，简体中文在后；保留 0.3.1、0.3.0 与 0.2.0 历史 |
...
4. 上传相对本目录的 `../../dist/PartyTargetWatch-0.4.0.zip` 作为版本文件，选择 Retail、游戏版本 `12.1.0`，版本类型选择 **Beta**，更新说明使用 `changelog.md`。
5. 提交前核对预览与字段：游戏内 UI 为简体中文；0.4.0 有 76 个 Lua mock 通过（39 UI、37 通信），交付测试 7 通过、1 因 Windows 符号链接权限跳过。主动目标通报及相关绑定已经移除，ZIP 应包含 7 个文件且没有 `Bindings.xml`。用户已确认 `%f` 展开，并在第二轮确认本版正常显示标记图标和纯怪物名称；跨客户端、完整两行宏和战斗/M+效果未验证。
