...
这些文件用于准备 PartyTargetWatch 0.4.2 的 CurseForge 项目页与版本说明。
...
| `changelog.md` | 0.4.2 更新说明；英文在前，简体中文在后；保留 0.4.1 及更早历史 |
...
4. 上传相对本目录的 `../../dist/PartyTargetWatch-0.4.2.zip` 作为版本文件，选择 Retail、游戏版本 `12.1.0`，版本类型选择 **Beta**，更新说明使用 `changelog.md`。
5. 提交前核对预览与字段：游戏内 UI 为简体中文；0.4.2 已移除插件间焦点同步，只保留两个焦点/通报开关，并增加小地图按钮与原生设置分类。新版及两个入口尚待实机确认，当前离线检查见 `validation/TESTING.md`。0.4.0 的本机名称、图标和 SeUI 联动成功反馈仅为历史记录；跨客户端与战斗/M+效果未验证。ZIP 应包含 8 个文件（新增 `Integration.lua`）且没有 `Bindings.xml`。
