...
5. 提交前核对预览与字段：游戏内 UI 为简体中文；0.4.0 有 76 个 Lua mock 通过（39 UI、37 通信），交付测试 7 通过、1 因 Windows 符号链接权限跳过。主动目标通报及相关绑定已经移除，ZIP 应包含 7 个文件且没有 `Bindings.xml`。用户已确认原生 `%f` 展开及本版名称接收/显示；前置标记清理修正仍待实机复验，跨客户端和战斗/M+效果未验证。
